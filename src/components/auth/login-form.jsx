  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (!success) {
        ...
        setError('Email or password are incorrect. For predefined accounts: admin@arcade.com/admin123 or user@arcade.com/user123');
      } else {
      ...
      if (err.response?.status === 401) {
        ...
        const errorMessage = `Login error: ${err.response?.data?.message || err.message || 'Unknown error'}`;
        setError(errorMessage);
        toast({
